...
if __name__ == '__main__':
  jbig2 = os.path.dirname(os.path.abspath(__file__)) + '/jbig2enc/src/jbig2'
  if len(sys.argv) <= 2 or not sys.argv[1].isnumeric():
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
      ...
      sym = base + '.sym'
      if enc.returncode != 0:
        sys.stderr.write("jbig2 call failed...\n")
        sys.exit(enc.returncode)
      elif not os.path.exists(sym):
        sys.stderr.write("jbig2 call succeeded but no output...\n")
        sys.exit(1)
      else:
